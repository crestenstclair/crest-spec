...
project: assets: StandaloneUiMain: {
	...
	prompts: [
		...
		"HOST THE LIVE ENGINE — THE AUDIO PATH MUST ACTUALLY PRODUCE SOUND, not a stub. Open external MIDI input via the MidirInput adapter (Shell::MidiInput) and an audio output stream via the CpalAudioOutput adapter (Shell::AudioOutput). CpalAudioOutput::open_stream starts a cpal callback that drains the adapter's internal ring buffer; you MUST feed that buffer with rendered engine audio by calling AudioOutput::write_buffer(&[AudioFrame]) — opening the stream alone yields SILENCE. Spawn a dedicated audio-producer thread that loops: (1) drains pending MIDI events from BOTH sources — the external MidirInput AND the --play sequencer (see below) — and applies them to the VoiceAllocator (note-on allocates/steals a voice, note-off releases); (2) renders a block of AudioFrames via VoiceAllocator -> SynthEngine -> PatchMixer -> GlobalMixer, applying the current ParameterSnapshot (master gain, etc.); (3) calls cpal_out.write_buffer(&block) to hand the audio to the stream; pace it so the ring buffer stays fed (e.g. render fixed-size blocks and sleep ~block duration, or block on buffer space). The VoiceAllocator/SynthEngine/mixers are OWNED and DRIVEN by this loop — they must NOT be unused/`_`-prefixed bindings. The UI thread only edits parameters (publishing the ParameterSnapshot the audio loop reads) and renders the view; it originates no notes.",

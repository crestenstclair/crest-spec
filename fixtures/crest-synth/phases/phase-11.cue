...
project: assets: StandaloneUiMain: {
	...
	prompts: [
		...
		#"AUDIO FEED RATE — CRITICAL, this is what makes it sound right vs. an "incredibly low" buzzing tone: the cpal callback consumes sample_rate frames PER SECOND (e.g. 48000/s), but the eframe update tick only runs ~60 times/sec. So you must feed ~sample_rate/60 frames PER TICK, NOT a fixed tiny block. Feeding a small constant block (e.g. 64 frames = ~1.3 ms) each ~16.7 ms repaint starves the ring buffer ~90% of the time; the callback fills the gaps with zeros, producing a ~60 Hz amplitude-gating artifact (a very low ~B1 buzz) over the real notes. Instead, pace by REAL TIME: track the wall-clock instant of the last feed; each tick compute elapsed = now - last, render round(elapsed_secs * sample_rate) frames (in BLOCK_SIZE chunks), call write_buffer, and set last = now. Cap a single tick's render to the ring-buffer capacity to avoid runaway after a stall, and prime the buffer once at startup with a chunk of audio. Average production then equals consumption, the buffer stays full, and the gating buzz disappears. (The ring buffer absorbs normal UI jitter; if you later want glitch-free audio under heavy UI load, the engine render can move into the cpal callback itself — but the real-time-paced main-thread feed is the MVP.)"#,

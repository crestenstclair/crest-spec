...
project: assets: StandaloneUiMain: {
	...
	prompts: [
		...
		#"Optional `--play <FILE.mid>` flag (dev/audition convenience, NOT a product feature): on launch, load the given Standard MIDI File via the phase-1 MidiFileLoader and run a simple internal sequencer (background thread/timer) that feeds the file's note-on/note-off and other MidiEvents into the SAME engine input the external-MIDI path uses, at their scheduled times, looping when the file ends — so you hear the synth while editing without external gear. This does NOT change the architecture: external MIDI (MidirInput) is still opened and remains the primary note source; --play is just an additional internal MIDI source for monitoring. Keep the audio thread lock-free (cross the existing real-time seam). Without --play, behave exactly as before (external MIDI only). If the file is missing/unparseable, print a clear stderr warning and continue running the editor (do not crash)."#,

...
project: assets: StandaloneUiMain: {
	...
	prompts: [
		...
		#"THREADING — a cpal Stream / CpalAudioOutput is NOT Send on macOS (CoreAudio); you CANNOT move it into a thread::spawn closure (that fails to compile with "*mut () cannot be sent between threads safely"). So keep CpalAudioOutput on the thread that created it (the main/UI thread) and render+feed there. Concretely: own the VoiceAllocator/SynthEngine/PatchMixer/GlobalMixer and the CpalAudioOutput on the main thread, and in the eframe update tick (each frame) do: drain pending MIDI events, apply them to the VoiceAllocator (note-on allocates/steals a voice; note-off releases), render a block of AudioFrames via VoiceAllocator -> SynthEngine -> PatchMixer -> GlobalMixer applying the current parameter values (master gain, etc.), and call cpal_out.write_buffer(&block) to keep the ring buffer fed (request a repaint each frame so updates keep coming). These engine objects must be OWNED and DRIVEN here — never unused/`_`-prefixed. Do NOT spawn a thread that holds the stream or the engine."#,

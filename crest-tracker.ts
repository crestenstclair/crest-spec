...
// --- Mixer commands (on Track entities within Song) ---
...
// Filters
audioEffects.aggregate("LowPassFilter", { root: true, implements: audioTransform, state: { cutoff: "number", resonance: "number" } });
audioEffects.aggregate("HighPassFilter", { root: true, implements: audioTransform, state: { cutoff: "number", resonance: "number" } });
audioEffects.aggregate("BandPassFilter", { root: true, implements: audioTransform, state: { cutoff: "number", resonance: "number", bandwidth: "number" } });
audioEffects.aggregate("LadderFilter", { root: true, implements: audioTransform, state: { cutoff: "number", resonance: "number", drive: "number" } });
audioEffects.aggregate("ResonantFilterBank", { root: true, implements: audioTransform, state: { bands: "number[]" } });

// Envelopes
audioEffects.aggregate("AmpEnvelope", { root: true, implements: audioTransform, state: { attack: "number", decay: "number", sustain: "number", release: "number" } });
audioEffects.aggregate("FilterEnvelope", { root: true, implements: audioTransform, state: { attack: "number", decay: "number", sustain: "number", release: "number", depth: "number" } });

// Modulation
audioEffects.aggregate("Chorus", { root: true, implements: audioTransform, state: { rate: "number", depth: "number", mix: "number" } });
audioEffects.aggregate("Flanger", { root: true, implements: audioTransform, state: { rate: "number", depth: "number", feedback: "number", mix: "number" } });
audioEffects.aggregate("Phaser", { root: true, implements: audioTransform, state: { rate: "number", depth: "number", stages: "number", mix: "number" } });
audioEffects.aggregate("Tremolo", { root: true, implements: audioTransform, state: { rate: "number", depth: "number", waveform: "string" } });
audioEffects.aggregate("RingModulator", { root: true, implements: audioTransform, state: { frequency: "number", mix: "number" } });

// Distortion
audioEffects.aggregate("Waveshaper", { root: true, implements: audioTransform, state: { drive: "number", curve: "string", mix: "number" } });
audioEffects.aggregate("Bitcrusher", { root: true, implements: audioTransform, state: { bits: "number", sampleReduction: "number" } });
audioEffects.aggregate("DualBandDistortion", { root: true, implements: audioTransform, state: { crossover: "number", lowDrive: "number", highDrive: "number" } });

// Delay
audioEffects.aggregate("Delay", { root: true, implements: audioTransform, state: { time: "number", feedback: "number", mix: "number" } });

// Dynamics
audioEffects.aggregate("Compressor", { root: true, implements: audioTransform, state: { threshold: "number", ratio: "number", attack: "number", release: "number", makeupGain: "number" } });
audioEffects.aggregate("Gate", { root: true, implements: audioTransform, state: { threshold: "number", attack: "number", release: "number" } });

// EQ
audioEffects.aggregate("ThreeBandEQ", { root: true, implements: audioTransform, state: { low: "number", mid: "number", high: "number", lowFreq: "number", highFreq: "number" } });
...
performance.aggregate("ChordPad", {
  root: true,
...
midiImport.aggregate("ImportPipeline", {
  root: true,

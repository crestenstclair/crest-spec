...
1. **Amendments live on the shared `Meta` struct** (`internal/cue/types.go`), authored in CUE as `meta: { amendments: [...] }` on any object. They are **not** merged/inherited by `mergeMeta` (no cascading a context's amendment to its children — amendments are targeted per-resource). A generic helper `ResourceAmendments(Resource) []Amendment` type-switches `Declaration` and returns that declaration's **own** `Meta.Amendments`, keeping reading consistent with what `declHash` marshals. **All CUE write-back/rendering emits `<path>: meta: amendments: [...]`** (not a top-level `amendments:` field).

...
To AUTHOR or modify the spec (.cue files), use the "spec-authoring" skill — it
holds the vision and methodology (specs describe WHAT; the loop writes the code;
group by domain; never hand-write target code). To RUN generation, use the
"spec-generate" skill / workflow below.

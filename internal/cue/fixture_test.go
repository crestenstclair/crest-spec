...
func TestGoalOrientedCrestSynthExampleLoadsAndTraces(t *testing.T) {
	...
	require.Contains(t, project.Completion.ProjectChecks, "validation.audio_integration")
	require.Len(t, project.Validations, 2)
	require.Equal(t, "integration_wave", project.Validations[0].Scope)
	require.Contains(t, project.Witnesses, "audible_a4")
	require.Contains(t, project.Witnesses["audible_a4"].Resources, "asset.ToneWitness")

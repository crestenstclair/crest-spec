...
Invariants are checked structurally at plan time and injected into every generation prompt (system prompt + `spec/context`'s `Invariants`) as hard constraints, `rationale` included, so the sub-agent builds against *why* the rule exists, not just the rule. Verification against generated code is independent: the behavioral pipeline's falsification-gated checks (§ gate-hardening revision), never a generator's own verdict.
...
+-------------+-----------------------------------------------+
              ...
              v
+-------------------------------------------------------------+
|         Commit Gate (server — mechanical, fail-closed)      |
...
| `rejected` | The commit gate failed (a mechanical validation failed or could not launch). The failure is stored and injected into the next `spec/context`. **Not terminal** — the workflow retries, then triages. |
...
Adding an amendment changes the resource's **declaration hash**, so the planner emits an `ActionModify` for that resource and it regenerates (see section 5.1). This rides the normal spec-hash cascade — hand-edits on disk are not tracked at all (content-drift detection was removed).
...
| `spec/promote_learnings` | Preview operational learnings selected by confidence/use thresholds. Threshold-only apply is rejected. |
| `spec/evaluation_cases`, `spec/evaluation_case_from_attempt`, `spec/evaluation_case_curate` | List/inspect immutable cases or capture a reproducible historical/curated case in SQLite. |
| `spec/evaluation_datasets`, `spec/evaluation_dataset_create`, `spec/evaluation_dataset_add_case`, `spec/evaluation_dataset_seal` | Build and seal immutable training/development/held-out datasets. |
| `spec/evaluation_configurations`, `spec/evaluation_configuration_create` | List/inspect or create immutable redacted comparison configurations. |
| `spec/evaluation_runs`, `spec/evaluation_run_create`, `spec/evaluation_run_finalize` | Create, inspect, aggregate, and compare baseline/candidate runs. |
| `spec/evaluation_assignment_claim`, `spec/evaluation_assignment_heartbeat`, `spec/evaluation_assignment_release`, `spec/evaluation_assignment_cancel`, `spec/evaluation_assignment_submit` | Drive leased host assignments; authoritative submission requires an ordinary terminal generation attempt. |
| `spec/evaluation_promotions`, `spec/evaluation_learning_propose`, `spec/evaluation_promotion_decide`, `spec/evaluation_promotion_apply` | Inspect or drive evidence-gated, human-approved reusable improvement with exact rollback identity. |
